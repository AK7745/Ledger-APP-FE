...
import type { Bill, Item, Party } from '@/lib/types';
...
export default function BillEditor({ bill }: { bill?: Bill }) {
  ...
  const [partyId, setPartyId] = useState(bill?.partyId ?? '');
  const [supplierRef, setSupplierRef] = useState(bill?.supplierRef ?? '');
  const [issueDate, setIssueDate] = useState(bill?.issueDate?.slice(0, 10) ?? today());
  const [dueDate, setDueDate] = useState(bill?.dueDate?.slice(0, 10) ?? '');
  const [notes, setNotes] = useState(bill?.notes ?? '');
  const [billDiscount, setBillDiscount] = useState(bill ? String(Number(bill.billDiscount)) : '0');
  const [lines, setLines] = useState<EditorLine[]>(
    bill?.lines.length
      ? bill.lines.map((l) => ({
          key: nextKey(), itemId: l.itemId ?? '', description: l.description,
          qty: String(Number(l.qty)), unitPrice: String(Number(l.unitPrice)), discount: String(Number(l.discount)),
        }))
  ...
  const grandTotal = subtotal - num(billDiscount);
  ...
  async function save() {
    ...
    setSaving(true);
    const payload = {
      partyId, supplierRef: supplierRef || undefined, issueDate,
      dueDate: dueDate || undefined, notes: notes || undefined,
      billDiscount: num(billDiscount),
      lines: lines.map((l) => ({
    ...
    try {
      const saved = editing ? await api.patch<Bill>(`bills/${bill!.id}`, payload) : await api.post<Bill>('bills', payload);
    ...
    <div className="space-y-4">
      <Card>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Supplier's invoice no." hint="the vendor's own reference">
              ...
              <tr key={l.key} className="border-t border-gray-100">
                <td className="py-2 pr-2">
                  ...
                  <Input value={l.description} placeholder="Description"
                    onChange={(e) => updateLine(l.key, { description: e.target.value })} className="mt-0" />
                </td>
      ...
      <div className="flex gap-4">
        <Card>
          <Field label="Notes"><Textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} className="w-72" /></Field>
        </Card>
      ...
      <div className="flex gap-2">
        <Button onClick={save} disabled={saving}>{saving ? 'Saving…' : editing ? 'Save draft' : 'Create draft'}</Button>
        <LinkButton href={editing ? `/bills/${bill!.id}` : '/bills'} variant="secondary">Cancel</LinkButton>
      </div>
    </div>
  );
}
